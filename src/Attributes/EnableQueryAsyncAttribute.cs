...
public sealed class EnableQueryAsyncAttribute<T> : Attribute, IAsyncActionFilter
{
    private readonly int _topMax = 100;

    public EnableQueryAsyncAttribute(int topMax)
    {
        _topMax = topMax;
    }

    public EnableQueryAsyncAttribute() { }
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext _, ActionExecutionDelegate next)
    {
        ...
        bool parsedTop = int.TryParse(topString, out int top);
        ...
        try
        {
            var (queryableData, totalCount) = await content.ApplyAsync(query, _topMax, searchBinder);

            var data = await ((IQueryable<object>)queryableData).ToListAsync();
            ...
            result.Value = new PagedResponse<object>(data, totalCount);
        }
        catch (Exception ex)
        {
            result.StatusCode = StatusCodes.Status400BadRequest;
