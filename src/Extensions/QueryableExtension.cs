...
public static class QueryableExtension
{
    ...
    public static async Task<(IQueryable, int?)> ApplyAsync<T>(this IQueryable<T> queryable, Query query, int? maxTop = null, ISearchBinder<T>? searchBinder = null)
    {
        var result = (IQueryable)queryable;
        ...
        if (query.Count ?? false)
        {
            ...
            {
                count = await ((IQueryable<object>)result).CountAsync();
            }
            else
            {
                count = result.Count();
